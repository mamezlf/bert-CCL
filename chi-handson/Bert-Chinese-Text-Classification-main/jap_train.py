...
best_model_path = os.path.join(save_path, 'best_model_state.bin')  # 已保存的模型路径
...
# 开始增量训练
print("日语ニュース分類の追加トレーニングを開始します...")
for epoch in range(5):  # 假设继续训练5个epoch
    model.train()
    epoch_loss = 0
    total_acc_train = 0
    ...
    if test_accuracy > best_test_accuracy:
        ...
        torch.save(model.state_dict(), best_model_path)  # 继续覆盖保存模型
        print(f"エポック {epoch + 1}: モデルを保存しました（テスト精度: {test_accuracy:.3f}）")

print("追加トレーニングが完了しました。")
